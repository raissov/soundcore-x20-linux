...
# Язык интерфейса: en, ru. Пусто — следовать системной локали.
LANGUAGE=
```

## Язык

Интерфейс следует за системной локалью: английская система — английский
интерфейс, русская — русский. Определяется по `LANGUAGE`/`LC_MESSAGES`/`LANG`,
при незнакомом языке откат на английский. Принудительно — строкой `LANGUAGE=`
в конфиге или переменной `HP_LANG` (например `HP_LANG=ru`).

Строки лежат в `i18n/<код>.json` — один каталог на всё: shell-скрипты,
Python и веб-страницы читают его же. Добавить язык — скопировать `en.json`
и перевести значения; новый файл подхватится сам.
